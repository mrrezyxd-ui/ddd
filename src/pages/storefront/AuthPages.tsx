...
export const AuthPages: React.FC<AuthPagesProps> = ({
  ...
  const handleDiscordAuth = async () => {
    ...
    setError(null);
    try {
      // Use localhost/callback as requested, or dynamic origin if available
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === 'register') {
        ...
        const res = await api.customerRegister(username, email, password);
      ...
      <div className="w-full max-w-md space-y-6">
        ...
        <div className="text-center space-y-2">
          ...
          <p className="text-xs text-zinc-400">
            {mode === 'register'
              ? 'Keep track of all your license keys and orders in one place.'
              : 'Log in to access your digital order history.'}
          </p>
        </div>

        {/* Card */}
        <div className="rounded-2xl border border-zinc-800 bg-zinc-900/90 p-6 sm:p-8 shadow-2xl backdrop-blur-md space-y-5">
          {error && (
          ...
          {/* Switch Mode Footer */}
          <div className="border-t border-zinc-800/80 pt-4 text-center text-xs text-zinc-400">
            {mode === 'register' ? (
              <p>
